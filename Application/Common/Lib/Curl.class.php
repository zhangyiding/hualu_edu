...
class Curl {
	...
	*/
	public static function post($url, $data, &$result, $timeout = 20) {
		...
		$result = curl_exec($ch);
        $end_time = microtime(true);
        RecordLog::add_curl_log($ch, $url,$start_time, $end_time, $data, 'post',$result);
	...
	*/
    ...
    public static function curl_post_xml($url, $xml_data, $timeout = 1) {
        ...
        $end_time = microtime(true);
        RecordLog::add_curl_log($ch, $url, $start_time, $end_time,'post',$response);
        if (!$response) {
            $curl_error = curl_error($ch);
            if (class_exists('RecordLog', false)) {
                RecordLog::addLog('请求接口：' . $url . ',错误信息：' . $curl_error . ',请求参数:'.  $xml_data);
            }
        }
