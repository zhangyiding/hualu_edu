...
class Verify {
    protected $config =	array(
        'seKey'     =>  'tcago_api_pic_auth',   // 验证码加密密钥
     ...
     * @return bool 用户验证码是否正确
     */
    public function check_new($code, $id = '') {
    	...
    	if(strtoupper(implode('', $code)) == $secode['verify_code']) {
    		$this->reset && $redis->remove($key);
    		return true;
    	}
    ...
    public function entry_new($id = '') {
    	...
    	$redis->connect('db1','1');
    	$catch_key = $key.$id;
    	$redis->set($catch_key, json_encode($secode));
